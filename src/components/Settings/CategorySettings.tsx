...
import { Tag, Plus, Trash2, ArrowUp, ArrowDown, X, Save, Pencil, Check } from 'lucide-react';
...
export function CategorySettings() {
  const [rows, setRows] = useState<ModelCategoryRow[]>([]);
  ...
  const refreshStore = useModelCategoryStore((s) => s.fetchCategories);

  const load = async () => {
    try {
      const data = await svc.getModelCategories();
      setRows(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await svc.addModelCategory(newName);
      ...
      addToast('Erreur : cette catégorie existe déjà', 'error');
    }
  };

  const handleSaveRename = async (row: ModelCategoryRow) => {
    const clean = editingName.trim();
    if (!clean || clean === row.name) { setEditingId(null); return; }
    try {
      await svc.updateModelCategory(row.id, clean, row.name);
  ...
  const handleDelete = async (row: ModelCategoryRow) => {
    const ok = await openConfirm(
      `Supprimer la catégorie "${row.name}" ?`,
      'Si elle est utilisée par des modèles, la suppression sera bloquée.'
    );
    if (!ok) return;
    try {
      await svc.deleteModelCategory(row.id, row.name);
  ...
  const handleSaveOrder = async () => {
    setSaving(true);
    try {
      await svc.reorderModelCategories(rows);
  ...
  return (
    <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-serif italic text-[#1C1917]">Catégories de Modèles</h3>
          <p className="text-[10px] font-black uppercase tracking-widest text-[#B68D40]">Personnalisez les types de confection</p>
        </div>
        <button
      ...
      <div className="bg-white border border-[#E7E5E4] rounded-[2.5rem] overflow-hidden shadow-sm">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-[#FAF9F6]">
            ...
            {rows.length === 0 && (
              <tr><td colSpan={3} className="px-8 py-12 text-center text-[#A8A29E] text-xs">Aucune catégorie. Ajoutez-en une.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="flex justify-between items-center bg-[#FAF9F6] p-8 rounded-[2.5rem] border border-[#E7E5E4]">
        <div className="flex items-center gap-4 text-[#78716C]">
          ...
          <p className="text-[10px] uppercase tracking-widest font-black max-w-xs leading-relaxed">
            L'ordre définit l'affichage dans les formulaires de catalogue et de commande.
          </p>
        </div>
        <button
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-[#1C1917]/20 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-8 animate-in zoom-in-95 duration-500">
            <div className="flex items-center justify-between mb-8">
              <h4 className="text-xl font-serif italic">Nouvelle catégorie</h4>
